...
const PurchaseSuccessPage = () => {
  ...
  useEffect(() => {
    const handleCheckoutSuccess = async (sessionId) => {
      try {
        console.log("Handling checkout success for session ID:", sessionId);
        await axios.post("/payments/checkout-success", { sessionId });
        console.log("Clearing the cart...");
        clearCart();
      } catch (error) {
        console.log("Error handling checkout success: ", error);
      } finally {
      ...
      <div className="max-w-md w-full bg-gray-800 rounded-lg shadow-xl overflow-hidden relative z-10">
        <div className="p-6 sm:p-8">
          ...
          <div className="space-y-4">
            ...
            <Link
              to="/"
              className="w-full bg-gray-700 hover:bg-gray-600 font-bold py-2 px-4 rounded-lg transition duration-300 flex items-center justify-center"
            >
              Continue Shopping
